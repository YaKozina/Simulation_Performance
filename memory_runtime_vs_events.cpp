void memory_runtime_vs_events() {
    ...
    // -------------------- CANVAS --------------------
    TCanvas *c1 = new TCanvas("c1", "Memory and Runtime vs Events", 1200, 600);
    c1->Divide(2,1);

    // -------------------- LEFT: Memory vs Events --------------------
    c1->cd(1);
    ...
    mg_mem->SetTitle("Memory [MiB] vs n of Events/Job;Events per Job;Memory (MB)");
    mg_mem->Draw("A PMC");
    leg_mem->Draw();

    // -------------------- RIGHT: Runtime [MiB] vs Events --------------------
    c1->cd(2);
    ...
    mg_time->SetTitle("Runtime [s] vs Events/Job;Events per Job;Runtime (s)");
    mg_time->Draw("A PMC");
    leg_time->Draw();
